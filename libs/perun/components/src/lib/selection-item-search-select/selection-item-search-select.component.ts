...
export class SelectionItemSearchSelectComponent implements OnInit{
  ...
  createSelectionItems() {
    ...
    for (const attribute of this.attributes) {
      const item: SelectionItem = new SelectionItem(attribute.friendlyName + ' (' + attribute.entity + ' / ' + SelectionItemSearchSelectComponent.getDefinition(attribute) + ')',
      ...
      if (attribute.entity.toLowerCase() === 'user' || attribute.entity.toLowerCase() === 'member') {
        ...
        this.items.push(item);
      } else if (attribute.entity.toLowerCase() === 'group' && this.asGroup && this.type === ItemType.SOURCE) {
  ...
  getFederationAttributes() {
    this.translateService.get('DIALOGS.APPLICATION_FORM_EDIT_ITEM.NO_SELECTED_ITEM').subscribe( noItem => {
      ...
      this.translateService.get('DIALOGS.APPLICATION_FORM_EDIT_ITEM.CUSTOM_VALUE').subscribe( custom => {
        ...
        this.items.push(new SelectionItem('EduPersonScopedAffiliation', 'eduPersonScopedAffiliation'));
        this.items.push(new SelectionItem('Forwarded Affiliation from Proxy', 'forwardedScopedAffiliation'));
