...
export class AuditMessagesListComponent implements OnInit, OnChanges, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    ...
    merge(this.sort.sortChange, this.child.paginator.page)
      .pipe(
        tap(() => this.loadAuditMessagesPage())
      )
      .subscribe();
  }
  ...
  ngOnInit(): void {
    ...
    this.dataSource.loadAuditMessages(this.tableConfigService.getTablePageSize(this.tableId),0,'DESCENDING');
  }
  ...
  loadAuditMessagesPage() {
    ...
    this.dataSource.loadAuditMessages(this.child.paginator.pageSize,this.child.paginator.pageIndex,sortDirection);
  }
  ...
  exportData(format: string) {
    downloadData(getDataForExport(this.dataSource.getData(), this.displayedColumns.filter(v => v !== 'detail'), this.getExportDataForColumn, this), format);
  }
